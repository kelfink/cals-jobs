...
public class InitialAndIncrementalFacilityLoadTest extends BaseApiTest<TestCalsJobsConfiguration> {
  ...
  private void incrementalLoadTest()
      throws IOException, LiquibaseException, JSONException, InterruptedException {
    ...
    target = clientTestRule.target(PATH_CHANGED_FACILITY + "/?" + DATE_AFTER + "=" + dateAfter
        .format(TimestampOperator.DATE_TIME_FORMATTER));
    CollectionDTO<TestChangedFacilityDTO> facilities = getFacilities(target);
    assertEquals(4, facilities.getCollection().size());
    assertFacility(facilities, CWSCMS_INCREMENTAL_LOAD_NEW_FACILITY_ID,
        "fixtures/cwsrs_new_facility.json");
    assertFacility(facilities, CWSCMS_INCREMENTAL_LOAD_UPDATED_FACILITY_ID,
        "fixtures/cwsrs_updated_facility.json");
    assertFacility(facilities, CWSCMS_INCREMENTAL_LOAD_DELETED_FACILITY_ID,
        "fixtures/cwsrs_deleted_facility.json");
    assertFacility(facilities, LIS_INCREMENTAL_LOAD_UPDATED_FACILITY_ID,
        "fixtures/lis_updated_facility.json");
  }

  private void assertFacility(CollectionDTO<TestChangedFacilityDTO> facilities, String facilityId,
      String fixturePath)
  ...
  private void addCwsDataForIncrementalLoad() throws LiquibaseException {
    ...
    DataSourceFactory cwsDataSourceFactory = application.getConfiguration()
        .getCmsrsDataSourceFactory();
  ...
  private void addLisDataForIncrementalLoad() throws LiquibaseException {
    DataSourceFactory lisDataSourceFactory = application.getConfiguration()
  ...
  private void initialLoadTest() throws IOException, JSONException {
    ...
    List<TestChangedFacilityDTO> filteredList =
        facilities.getCollection().stream().filter(
            (o) -> CWSCMS_INITIAL_LOAD_FACILITY_ID.equals(o.getId()) || LIS_INITIAL_LOAD_FACILITY_ID
                .equals(o.getId()))
            .collect(Collectors.toList());
    Optional<TestChangedFacilityDTO> cwsCmsFacility =
        filteredList.stream().filter((o) -> CWSCMS_INITIAL_LOAD_FACILITY_ID.equals(o.getId()))
            .findAny();
    Optional<TestChangedFacilityDTO> lisFacility =
        filteredList.stream().filter((o) -> LIS_INITIAL_LOAD_FACILITY_ID.equals(o.getId()))
            .findAny();
    JSONAssert.assertEquals(
  ...
  private static CollectionDTO<TestChangedFacilityDTO> getFacilities(WebTarget target)
      throws IOException {
