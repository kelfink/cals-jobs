...
//@formatter:off
  public String getPrepLastChangeSQL() {
    return "INSERT INTO GT_ID (IDENTIFIER)"
        ...
        + "SELECT CAS.IDENTIFIER "
        + "\nFROM CASE_T CAS"
        + "\nLEFT JOIN CHLD_CLT CCL1 ON CCL1.FKCLIENT_T = CAS.FKCHLD_CLT  AND CCL1.IBMSNAP_OPERATION IN ('I','U') "
        + "\nLEFT JOIN CLIENT_T CLC1 ON CLC1.IDENTIFIER = CCL1.FKCLIENT_T AND CLC1.IBMSNAP_OPERATION IN ('I','U') "
        + "\nWHERE CCL1.IBMSNAP_LOGMARKER > ? "
        + "\nAND CAS.IBMSNAP_OPERATION IN ('I','U') "
        + "\nUNION"
        + "\n SELECT CAS.IDENTIFIER "
        + "\n FROM CASE_T CAS "
        + "\nLEFT JOIN CHLD_CLT CCL2 ON CCL2.FKCLIENT_T = CAS.FKCHLD_CLT  AND CCL2.IBMSNAP_OPERATION IN ('I','U') "
        + "\nLEFT JOIN CLIENT_T CLC2 ON CLC2.IDENTIFIER = CCL2.FKCLIENT_T AND CLC2.IBMSNAP_OPERATION IN ('I','U') "
        + "\nWHERE CLC2.IBMSNAP_LOGMARKER > ? "
        + "\nAND CAS.IBMSNAP_OPERATION IN ('I','U') "
        + "\nUNION "
        + "\nSELECT CAS.IDENTIFIER "
        + "\nFROM CASE_T CAS "
        + "\nLEFT JOIN CHLD_CLT CCL3 ON CCL3.FKCLIENT_T = CAS.FKCHLD_CLT  AND CCL3.IBMSNAP_OPERATION IN ('I','U') "
        + "\nLEFT JOIN CLIENT_T CLC3 ON CLC3.IDENTIFIER = CCL3.FKCLIENT_T AND CCL3.IBMSNAP_OPERATION IN ('I','U') "
        ...
        + "\nSELECT CAS.IDENTIFIER "
        + "\nFROM CASE_T CAS "
        + "\nLEFT JOIN CHLD_CLT CCL ON CCL.FKCLIENT_T = CAS.FKCHLD_CLT AND CCL.IBMSNAP_OPERATION IN ('I','U') "
        + "\nLEFT JOIN CLIENT_T CLC ON CLC.IDENTIFIER = CCL.FKCLIENT_T AND CLC.IBMSNAP_OPERATION IN ('I','U') "
