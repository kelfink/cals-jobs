...
/**
 * Job to load attornies from CMS into ElasticSearch
 * 
 * @author CWDS API Team
 */
public class AttorneyIndexerJob extends JobBasedOnLastSuccessfulRunTime {
  ...
  public AttorneyIndexerJob(AttorneyDao attorneyDao, ElasticsearchDao elasticsearchDao,
  ...
  public static void main(String... args) throws Exception {
    if (args.length != 2) {
      throw new Error(
          "Usage: java gov.ca.cwds.jobs.AttorneyIndexerJob esconfigFileLocation lastJobRunTimeFilename");
    }
    ...
    AttorneyDao attorneyDao = new AttorneyDao(sessionFactory);
    ...
    AttorneyIndexerJob job = new AttorneyIndexerJob(attorneyDao, elasticsearchDao, args[1]);
    try {
      job.run();
    ...
    try {
      ...
      for (Attorney attorney : results) {
        gov.ca.cwds.rest.api.elasticsearch.ns.Person esPerson =
            new gov.ca.cwds.rest.api.elasticsearch.ns.Person(attorney.getPrimaryKey(),
                attorney.getFirstName(), attorney.getLastName(), "", // Gender
                nodate, // DOB
                "", // SSN
                attorney.getClass().getName(), MAPPER.writeValueAsString(attorney));
