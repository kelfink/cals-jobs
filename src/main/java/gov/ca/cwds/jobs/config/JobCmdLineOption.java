...
public enum JobCmdLineOption {
   ...
   */
  ES_CONFIG(JobOptions.makeOpt("c", JobOptions.CMD_LINE_ES_CONFIG,
      "ElasticSearch configuration file", true, 1, String.class, ',')),

  /**
   * ElasticSearch index name to create or use. If not provided then ES Config alias is used.
   */
  INDEX_NAME(JobOptions.makeOpt("i", JobOptions.CMD_LINE_INDEX_NAME, "ElasticSearch index name",
   ...
   */
  BUCKET_RANGE(JobOptions.makeOpt("r", JobOptions.CMD_LINE_BUCKET_RANGE, "bucket range (-r 20-24)",
      false, 2, Integer.class, '-')),

  /**
   * Total buckets.
   */
  BUCKET_TOTAL(JobOptions.makeOpt("b", JobOptions.CMD_LINE_BUCKET_TOTAL, "total buckets", false, 1,
      Integer.class, ',')),

  /**
   * Number of threads (optional).
   */
  THREADS(JobOptions.makeOpt("t", JobOptions.CMD_LINE_THREADS, "# of threads", false, 1,
      Integer.class, ',')),

  /**
   * Minimum key, inclusive.
   */
  MIN_ID(JobOptions.makeOpt("m", JobOptions.CMD_LINE_MIN_ID, "minimum identifier, inclusive", false,
      1, String.class, ',')),
