    ...
    for (int batchNumber = 0; batchNumber < jobBatches.size(); batchNumber++) {
      ...
      if (!JobExceptionHandler.isExceptionHappened()) {
        ...
        if (!jobBatches.get(batchNumber).isEmptyTimestamp()) {
          LOGGER.info("Save point has been reached. Save point batch timestamp is " + jobBatches
              .get(batchNumber).getTimestamp());
        }
      } else {
        LOGGER.error("Exception occured during batch processing. Job has been terminated." +
            " Batch timestamp " + jobBatches.get(batchNumber).getTimestamp()
